...
module.exports = {
  get: async (req, res) => {
    try {
      ...
      if(!filteredSaying) return res.status(404).json({ message: 'No Matched Saying!' });
      // 만약 요청된 명언 id와 일치하는 명언이 있다면, 해당 명언을 전달한다
      else return res.status(200).json({ data: { filteredSaying: filteredSaying }, message: 'Matched Saying!' });
    } catch (err) {
