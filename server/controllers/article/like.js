...
module.exports = {
  get: async (req, res) => {
    try {
      ...
      // articleId가 전달되지 않았다면, 다음을 응답한다
      if(!articleId) return res.status(400).json({ message: 'Bad Request!' });
      // 유저가 해당 게시글에 좋아요를 눌렀는지 확인
      const articleLikeInfo = await article_likes.findOne({ where: { user_id: userInfo.id, article_id: Number(articleId) } });
  ...
  delete: async (req, res) => {
    try {
      ...
      if(articleLikeInfo) {
        ...
        article_likes.destroy({ where: { user_id: userInfo.id, article_id: Number(articleId) } });
