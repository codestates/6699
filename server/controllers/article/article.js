const { articles, article_likes, comments } = require('../../models');
...
module.exports = {
  ...
  get: async (req, res) => {
    try {
      ...
      else return res.status(200).json({ data: { articleInfo: articleInfo }, message: 'Matched Article!' });
    } catch (err) {
      return res.status(500).json({ message: 'Server Error!' });
    }
  },
  ...
  delete: async (req, res) => {
    try {
      ...
      // 만약 일치하는 게시물이 존재하면, 다음을 실행한다
        ...
        articles.destroy({ where: { id: Number(articleId), user_id: user_id }});
